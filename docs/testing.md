...
- 消费人格画像、财务健康评分、问卷自评对比和关键边界条件可以基于当前用户数据生成。
- 家庭组织的 owner/admin/member 权限、家庭 Dashboard 和家庭消费人格聚合有效。
...
- `test_personality.py`: 问卷与交易画像偏差分析的中间带判断、openness 新商户比例的时间参考边界。
- `test_organizations.py`: 家庭组织创建、成员权限、家庭聚合和个人模式兼容。
...
当前报告整理时的验证结果：执行 `npm ci` 安装依赖后，`npm run build` 通过，`vue-tsc --noEmit` 和 Vite production build 均完成；Vite 提示生产 chunk 超过 500 KB，属于后续性能优化项。
...
11. 进入消费人格页面，检查人格画像、财务健康评分、维度解释和问卷自评对比。
12. 进入家庭组织页面，创建家庭、添加成员，检查家庭 Dashboard 和家庭消费人格聚合。
13. 进入系统设置，确认 provider 和阈值配置展示正常。
14. 使用管理员账号进入系统设置，确认“一键重试历史超时”按钮可见；普通用户不可见。
...
- 窄屏下导航和内容不会明显重叠。
- 表格、筛选栏、按钮和标签文本不溢出。
- Dashboard、消费人格、家庭组织和报表图表可渲染。
- 家庭组织页面在无组织、已有组织和成员列表状态下均有明确反馈。
...
- 为认证、导入、交易、分类、消费人格和报表接口增加 API 测试。
- 为家庭组织的成员邀请、角色变更、组织报表生成补充更多边界测试。
