...
Boring Financial 是一个面向个人记账和消费分析场景的智能账单分类系统。项目针对市面记账软件自动分类语义不准、分类依据不透明、误判后难以修正的问题，提供“大模型语义初分 + 人工校正”的账单分析闭环，支持微信/支付宝账单导入、模型分类、人工校正、统计看板、消费人格分析、家庭组织聚合和 PDF 报表。

## 项目目标

- 提供完整的记账分析闭环：导入 -> 解析 -> 大模型初分 -> 人工校正 -> 分析 -> 报表。
- 在低配服务器上保持可运行，优先复用已有聚合接口和前端派生展示。
- 以大模型语义分类为主，保留规则分类兜底，同时支持 OpenAI-compatible API 和本地模型服务。
- 在校正后的交易数据基础上提供个人 Dashboard、消费人格画像、财务健康评分、家庭聚合视图和 PDF 报表。

## 核心功能

- 多用户注册、登录与账单隔离。
- 微信/支付宝账单文件导入、批次进度展示和批次删除。
- 大模型语义分类、规则兜底和分类缓存组成的混合分类链路。
- 交易列表筛选：分页、平台、日期、分类、导入文件、待校正状态和关键词搜索。
- 分类校正工作台：查看模型建议、置信度、分类理由并人工确认。
- 分类管理：系统分类和用户自定义分类共存。
- Dashboard 财务驾驶舱：收入、支出、结余、储蓄率、Top 商户、分类占比、导入任务状态。
- 消费人格分析：基于交易数据生成消费人格画像、财务健康评分，并通过问卷自评展示数据画像与主观认知的差异。
- 家庭组织：创建家庭组织、管理成员角色，并按组织成员聚合 Dashboard、消费人格和重试/重分类操作。
...
- 分类管理：分类指标、新增分类、分类列表。
- 消费人格：16 型消费人格画像、财务健康评分、维度解释和问卷自评对比。
- 家庭组织：家庭成员管理、角色调整、家庭财务概览和家庭消费人格分析。
- 报表中心：条件选择、摘要预览、PDF 生成和预览下载。
- 系统设置：provider、阈值和模型配置展示。
...
当前仓库已经具备个人记账分析所需的主要产品闭环，并已完成生产环境部署。前端已重设计为深色侧边栏、白色工作台和绿色金融主色的后台系统，并补充了消费人格分析和家庭组织页面；后端接口保持稳定，适合本地试运行、低配服务器部署和生产环境运行。生产化继续增强时，优先补充前端端到端测试、报表历史列表、更细的分类规则管理、生产包拆分和家庭聚合性能优化。
